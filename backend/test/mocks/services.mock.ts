...
export const mockLLMService: LLMServiceMock = {
  ...
  callLLM: jest.fn().mockImplementation(async () => ({
    message: { content: JSON.stringify(expectedStoryResponse) },
  })),
  generateStoryInfo: jest
    .fn()
    .mockImplementation(async () => expectedStoryResponse),
  generateStoryImagePrompt: jest
    .fn()
    .mockImplementation(async () => 'test image prompt'),
  generateChapterContent: jest
    .fn()
    .mockImplementation(async () => ['Content 1', 'Content 2', 'Content 3']),
  getChapterBackgroundSound: jest
    .fn()
    .mockImplementation(async () => ({ sound: 'test sound' })),
  generateYouTubeMetadata: jest.fn().mockImplementation(async () => ({
    title: 'Test YouTube Title',
    description: 'Test YouTube Description',
    tags: ['test', 'tags'],
  })),
  generateChapterExceptForShort: jest
    .fn()
    .mockImplementation(async () => 'Test chapter excerpt'),
} as unknown as LLMServiceMock;

export const mockGenApiService: GenApiServiceMock = {
  generateStoryMedia: jest
    .fn()
    .mockImplementation(async () => expectedMediaResponse),
...
export const mockYoutubeService: YoutubeServiceMock = {
  uploadVideo: jest
    .fn()
    .mockImplementation(async () => ({ id: 'test-video-id' })),
  uploadShorts: jest
    .fn()
    .mockImplementation(async () => ({ id: 'test-shorts-id' })),
